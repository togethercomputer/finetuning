...
llm = AsyncLLM(api_provider="together", max_requests_per_minute=max_requests_per_minute, api_key=together_api_key)
openai_llm = AsyncLLM(api_provider="openai", max_requests_per_minute=max_requests_per_minute, api_key=openai_api_key)
...
openai_request_list = initial_request_list(openai_model, eval_data)
openai_completions = openai_llm.generate(openai_request_list)
...
openaiModelCompletions = [completion[1]['choices'][0]['message']['content'] for completion in openai_completions]
    ...
    results.append(
        {
            ...
            "openaiModelAnswer": openaiModelCompletions[idx]
        }
    )
...
request_list = prepare_request_list("baseModelAnswer", evaluator_model) + prepare_request_list("topOSSModelAnswer", evaluator_model) + prepare_request_list("fineTunedModelAnswer", evaluator_model) + prepare_request_list("openaiModelAnswer", evaluator_model)
...
for evaluation in evaluations:
    ...
    elif mak == "openaiModelAnswer":
        openai_model_evaluations.append(evaluation)
...
openaiModelCount = count_results(openai_model_evaluations)
...
print("OpenAI model count: ", openaiModelCount)
...
print("OpenAI model:", f"{openaiModelCount / len(results) *100 }%")